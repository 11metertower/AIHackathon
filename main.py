...
from mailClassification.crawl import save_recent_gmails_to_csv, EMAIL_ADDRESS, APP_PASSWORD
from mailClassification.classify import main as classify_main

def run_pipeline():
    print("🚀 AI Email Classification Pipeline 시작")
    
    # 1. 크롤링 (crawl.py의 기능을 호출하여 gmail_list.csv 생성)
    print("\n[Step 1] 이메일 데이터 수집 중...")
    save_recent_gmails_to_csv(
        EMAIL_ADDRESS, 
        APP_PASSWORD, 
        num_emails=100, 
        output_file="gmail_list.csv"
    )
